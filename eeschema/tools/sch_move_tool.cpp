    ...
    do
    {
                ...
                || evt->IsAction( &ACTIONS::refreshPreview ) )
        {
            ...
            if( updateBBox )
            {
                ...
                // A net label -- or a power port, which the same rule measures by its pin -- is
                // aligned to pins and other labels.  None of the three rules above accepts one,
                // so without this the drag produced an invalid box, lost its move context, and
                // got no guides at all.
